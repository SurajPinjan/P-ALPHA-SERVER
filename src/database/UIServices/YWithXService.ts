...
export const getallYWithX = async function (
  ...
  try {
    let _query: string = `SELECT y.*, x.uid as x_id, x.columnDate as x_columnDate, x.url as x_url, x.columnSelect as x_columnSelect FROM table_y y inner join table_x x on y.x_id = x.uid where y.isDeleted=false ${whereClause}`
